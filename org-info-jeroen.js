...
function OrgNode ( _div, _heading, _link, _depth, _parent, _base_id, _toc_anchor)
{
  ...
  t.DIRTY = false;                     // Node is dirty, when children get
                                       // folded seperatly.
  t.STATE = OrgNode.STATE_UNFOLDED;
  t.TOC   = _toc_anchor;
  ...
  //var folder = document.getElementById("outline-text-"+t.BASE_ID);
  var folder = null;
  if (t.DIV != null) {
    var folder = t.DIV.children[1]; // hard-code the text
  }
  if(null == folder && _base_id) {
    var fid =  _base_id.substring(4);
    folder = document.getElementById("outline-text-"+fid); // try old schema
  }
  if(null != folder)
    t.FOLDER = folder;
...
OrgNode.prototype.fold = function (hide_folder)
{
  ...
  if(null != this.FOLDER) {
    ...
    if(this.STATE == OrgNode.STATE_FOLDED) {
      ...
      // recursive. Jeroen: no not recursive...
      if(this.CHILDREN.length) {
        ...
        for(var i=0;i<this.CHILDREN.length;++i) { OrgNode.showElement(this.CHILDREN[i].DIV) }
      } else if (! hide_folder) {
    ...
    // }
      ...
      for(var i=0;i<this.CHILDREN.length;++i) { OrgNode.hideElement(this.CHILDREN[i].DIV) }
    }
  }
};
...
{
  var t = this;
  // Jeroen: I don't want the recursive behavior
